...
{
    [Authorize]
    public class AspNetCompanyFormModel : PageModel
    {
        private readonly IMapper _mapper;
        private readonly AspNetCompanyService _companyService;
        private readonly ICountryService _countrySevice;
        private readonly ICurrencyService _currencyService;
        private readonly ITimeZoneService _timeZoneService;
        public AspNetCompanyFormModel(
            IMapper mapper,
            AspNetCompanyService companyService,
        ...
        [BindProperty]
        public AspNetCompanyModel AspNetCompanyForm { get; set; } = default!;

        public class AspNetCompanyModel
        {
        ...
        public class MappingProfile : Profile
        {
            ...
            {
                CreateMap<AspNetCompany, AspNetCompanyModel>();
                CreateMap<AspNetCompanyModel, AspNetCompany>();
            }
        }
        ...
        public async Task OnGetAsync(Guid? rowGuid)
        {
            ...
            if (rowGuid.HasValue)
            {
                ...
                AspNetCompanyForm = _mapper.Map<AspNetCompanyModel>(existing);
            }
        }

        public async Task<IActionResult> OnPostAsync([Bind(Prefix = nameof(AspNetCompanyForm))] AspNetCompanyModel input)
        {
            ...
            if (action == "create")
            {
                var newobj = _mapper.Map<AspNetCompany>(input);
                await _companyService.AddAsync(newobj);

                this.WriteStatusMessage($"Success create new data.");
                return Redirect($"./AspNetCompanyForm?rowGuid={newobj.RowGuid}&action=edit");
            }
            else if (action == "edit")
            {
                ...
                return Redirect($"./AspNetCompanyForm?rowGuid={existing.RowGuid}&action=edit");
            }
            else if (action == "delete")
            {
                ...
                return Redirect("./AspNetCompanyList");
            }
            return Page();
        }
